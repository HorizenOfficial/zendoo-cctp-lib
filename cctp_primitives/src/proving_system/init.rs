...
#[cfg(test)]
/// In normal usage, `load_g{1,2}_committer_key()` should be called exactly once during program
...
/// - the unit test containing the call to `load_g{1,2}_committer_key()` would need to be run
...
/// are simple wrappers around `load_g{1,2}_committer_key()`, fixing a uniform value for
/// `max_degree` and catching (and discarding) the error generated by additional calls to
/// `load_g{1,2}_committer_key()`.
///
...
pub(crate) fn set_g1_universal_params_for_testing() {
    ...
    let result_g1 = load_g1_committer_key(max_degree);
    match result_g1 {
        Ok(_) => {
            // Do nothing.
        }
...
pub(crate) fn set_g2_universal_params_for_testing() {
    let max_degree = 1 << max_pow_for_testing();
    let result_g2 = load_g2_committer_key(max_degree);
    match result_g2 {
        Ok(_) => {
            // Do nothing.
        }
...
mod test {
    ...
    fn check_load_g2_committer_key() {
        ...
        load_g2_committer_key(max_degree).unwrap();
